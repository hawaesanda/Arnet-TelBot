...
def handle_stage(client, message):
    ...
    if user_team_verification == True:
        ...
        if message.text == "/selanjutnya":
            ...
            client.send_message(
                chat_id=user_id,
                text="Terjadi kesalahan. /restart untuk mengulangi"
                )
...
def handle_stage_team(client, message, stage):
    ...
    if stage == STAGE_TEAM:
        if message.text in ["/MAINTENANCE", "/PATROLI"]:
            ...
            client.send_message(
                chat_id=user_id,
                text="Silakan pilih jenis kegiatan: /MAINTENANCE   /PATROLI",
                reply_markup=get_navigation_buttons(STAGE_TEAM, user_id, message),
            )
...
def send_all_images(client, chat_id, folder_path):
    ...
    if not os.path.exists(folder_path):
        client.send_message(chat_id, "Belum ada laporan hari ini.")
        return
...
def load_text_from_file(data, user_id, team):
    ...
    if os.path.exists(file_path):
        with open(file_path, "r", encoding="utf-8") as file:
            content = file.read().strip()  # Membaca seluruh isi file dan menghilangkan spasi/enter di awal/akhir
