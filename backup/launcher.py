...
ADMIN_DAILY_DIR = "D:/Arnet-TelBot/admin_storage/daily-reports"
...
def handle_stage(client, message):
    ...
    if user_team_verification == True:
        ...
        if message.text == "/selanjutnya":
            ...
            if saved_user_id == True:
                send_all_images(client, message.chat.id)
...
#     print(user_attemp)
#     if user_attemp == 0:
#         client.send_message(message.chat.id, prompt_message)
#         if message.text == "/selanjutnya":
#             user_data[user_id]["stage"] -= 1
#             user_attemp =+ 1
#     else: 
#     # if img_user_verif == False:
#     #     client.send_message(message.chat.id, prompt_message)
#     #     if message.text == "/selanjutnya":
#     #         user_data[user_id]["stage"] -= 1
#     #         client.send_message(
...
#     # elif img_user_verif == True:
#     #     client.send_message(message.chat.id, prompt_message)
#     #     get_navigation_buttons(stage, user_id, message),

#         if message.text == "/selanjutnya":
#             img_user_verif = img_verification(user_dir)
#             print(img_user_verif)
#             if img_user_verif == True:
#                 get_navigation_buttons(stage, user_id, message),
#                 client.send_message(message.chat.id, prompt_message)
#             else:
#                 user_data[user_id]["stage"] -= 1
...
def handle_empty_stage(client, message, stage_name):
    ...
    user_id = message.from_user.id
    user_dir = os.path.join(BASE_TEMP_DIR, str(user_id), stage_name)
    # user_date_dir = os.path.join(BASE_TEMP_DIR,str(user_id), "Tanggal")

    if not os.path.exists(user_dir):
        os.makedirs(user_dir)
    
    # if not os.path.exists(user_date_dir):
    #     os.makedirs(user_date_dir)

    # Path untuk file .txt
    text_filename = os.path.join(user_dir, f"{stage_name}.txt")
    # user_date = os.path.join(user_date_dir, "Tanggal.txt")
    ...
    if message.text == "/bersihkan":
        ...
        if stage_name == str("Team"):
            client.send_message(
                message.chat.id,
                f"Pilih Team berikut :\n/SKSO /CME /IOTA\natau masukkan secara manual.",
            )
            # current_date = datetime.now()
            # nama_bulan = current_date.strftime("%B")
            # bulan_indonesia = bulan_dict[nama_bulan].upper()
            # current_date_formatted = current_date.strftime(f"%d {bulan_indonesia} %Y")  # Format DD MONTH YYYY
            # with open(user_date, "w", encoding="utf-8") as text_file:
            #     text_file.write(current_date_formatted)  # Menyimpan tanggal saat ini ke file .txt
        else:
            client.send_message(message.chat.id, f"Masukkan {stage_name}")

    elif message.text == "/start":
        client.send_message(
            message.chat.id, "Tombol navigasi tidak terdeteksi sebagai user input."
        )
    elif message.text == "/bantuan":
        client.send_message(
            # message.chat.id, "Navigation buttons do not count as text input."
        )
    elif message.text in ["/SKSO", "/CME", "/IOTA"]:
        userteaminput = message.text.strip("/")
        with open(text_filename, "w", encoding="utf-8") as text_file:
            text_file.write(userteaminput + "\n")  # Simpan teks ke file
        client.send_message(
            message.chat.id,
            f"Team {userteaminput} telah disimpan\n/selanjutnya untuk melanjutkan.",
        )
    else:
...
def save_image_admin(image, user_id):
    admin_img_path = datetime.now().strftime('%d%m%Y')
    admin_dir = os.path.join(ADMIN_DAILY_DIR, admin_img_path)
...
def send_all_images(client, chat_id):
    admin_img_path = datetime.now().strftime('%d%m%Y')
    admin_dir = os.path.join(ADMIN_DAILY_DIR, admin_img_path)
    # delete all report before
    daily_reset(ADMIN_DAILY_DIR)
    # Check if the folder exists
    if not os.path.exists(admin_dir):
        client.send_message(chat_id, "Laporan harian tidak tersedia.")
        return
    ...
    if not image_files:
        client.send_message(chat_id, "Tidak ada laporan harian hari ini.")
        return

    # Send each image file to the chat
    for image_file in image_files:
        image_path = os.path.join(admin_dir, image_file)
...
def daily_reset(admin_dir):
    today = datetime.now().strftime('%d%m%Y')
    # Menghapus semua folder kecuali folder untuk tanggal hari ini
    for folder_name in os.listdir(admin_dir):
        folder_path = os.path.join(admin_dir, folder_name)
        # Mengecek apakah item adalah direktori dan bukan folder tanggal hari ini
        if os.path.isdir(folder_path) and folder_name != today:
            # Menghapus folder
            shutil.rmtree(folder_path)
            print(f'Folder {folder_name} telah dihapus.')
    print(f'Folder selain {today} telah dibersihkan.')
